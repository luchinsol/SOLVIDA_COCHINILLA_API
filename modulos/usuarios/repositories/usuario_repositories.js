...
export const crearUsuarioRepo = async (usuario) => {
  const result = await db.one(
    ...
    [usuario.rol_id, usuario.nombre, usuario.correo, usuario.password, usuario.activo, usuario.nickname]
  )
  return result
}
...
export const actualizarUsuarioRepo = async (id, usuario) => {
  const result = await db.oneOrNone(
    ...
    [usuario.rol_id, usuario.nombre, usuario.correo, usuario.password, usuario.activo, usuario.nickname, id]
  )
  return result
}
